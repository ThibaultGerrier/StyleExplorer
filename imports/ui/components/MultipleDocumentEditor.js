  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <Dropzone onDrop={(acc, rej) => this.onDrop(acc, rej)} multiple={true} accept={'text/plain'} style={{
 borderWidth: '2px', borderColor: 'black', borderStyle: 'dashed', borderRadius: '4px', margin: '20px', padding: '20px', width: '100%', transition: 'all 0.5s',
}}>
        ...
        <ListGroup>
          {this.state.list.length !== 0 ?
            this.state.list.map(ele => (
              <ListGroupItem key={getIndex()}>
                <h4 className="list-group-item-heading" > {ele[0]} </h4>
                <Row>
